...
// In-memory, single-process — same assumption as scene-rate-limiter.js. Deliberately a
// SEPARATE module/counter (not the same Map) from scene-rate-limiter.js, so a caller's
// quota on /generate-ev-scene-v2 is independent of its quota on /generate-ev-scene-simple,
// even if the same `source` value is used for both.
const hourlyBySource = new Map() // source -> [timestamps]

const HOUR_MS = 60 * 60 * 1000

const LIMITS = {
  perSourcePerHour: parseInt(process.env.SIMPLE_SCENE_RATE_LIMIT_PER_SOURCE_HOUR || '20', 10),
}
...
function checkSimpleSceneRateLimit({ source }) {
...
module.exports = { checkSimpleSceneRateLimit, LIMITS }
