// render-service/test-scene-generator-simple.js
...
//   node test-scene-generator-simple.js   (in another, with the same RENDER_SECRET)
...
async function main() {
  ...
  const unauthorized = await axios.post(`${BASE_URL}/generate-ev-scene-simple`, {
  ...
  const resp = await axios.post(`${BASE_URL}/generate-ev-scene-simple`, {
    source: 'test-e2e',
  ...
  console.log('PASS — generated simple-scene image:', resp.data.image_url)
}
